...
type notify struct {
	connIdMap utils.ShardLockMaps
}

func NewNotify() biface.Inotify {
	return &notify{
		connIdMap: utils.NewShardLockMaps(),
	}
}
...
func (n *notify) SetNotifyId(Id uint64, conn biface.IConnection) {
	strId := n.genConnStrId(Id)
	n.connIdMap.Set(strId, conn)
}

func (n *notify) GetNotifyById(Id uint64) (biface.IConnection, error) {
...
func (n *notify) NotifyAll(MsgId uint32, data []byte) error {
	...
	n.connIdMap.IterCb(func(key string, v interface{}) {
		...
		err := conn.SendMsg(MsgId, data)
		if err != nil {
			blogger.Ins().ErrorF("Notify to %s err:%s \n", key, err)
		}
	})
...
func (n *notify) NotifyBuffToConnById(Id uint64, MsgId uint32, data []byte) error {
	...
	err = Conn.SendBufMsg(MsgId, data)
	if err != nil {
		blogger.Ins().ErrorF("Notify to %d err:%s \n", Id, err)
		return err
	}
	return nil
}
