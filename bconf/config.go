...
func UserConfToGlobal(config *Config) {
	...
	if config.MaxConnSize != 0 {
		GlobalObject.MaxConnSize = config.MaxConnSize
	}
	...
	if GlobalObject.LogIsolationLevel > blogger.LogDebug {
		blogger.SetLogLevel(GlobalObject.LogIsolationLevel)
	}
	...
	if config.LogFile != "" {
		...
		blogger.SetLogFile(GlobalObject.LogDir, GlobalObject.LogFile)
	}
