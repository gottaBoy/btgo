...
func NewHTLVCRCDecoder() biface.IDecoder {
	return &HtlvCrcDecoder{}
}

func (hcd *HtlvCrcDecoder) GetLengthField() *biface.LengthField {
	//+------+-------+---------+--------+--------+
	//| 头码  | 功能码 | 数据长度 | 数据内容 | CRC校验 |
	//| 1字节 | 1字节  | 1字节   | N字节   |  2字节  |
	//+------+-------+---------+--------+--------+
...
func (hcd *HtlvCrcDecoder) decode(data []byte) *HtlvCrcDecoder {
	...
	if !CheckCRC(data[:datasize-2], htlvData.Crc) {
		blogger.Ins().DebugF("crc check error %s %s\n", hex.EncodeToString(data), hex.EncodeToString(htlvData.Crc))
		return nil
	}

	//blogger.Ins().DebugF("2htlvData %s \n", hex.EncodeToString(htlvData.data))
	//blogger.Ins().DebugF("HTLVCRC-DecodeData size:%d data:%+v\n", unsafe.Sizeof(htlvData), htlvData)

	return &htlvData
}

func (hcd *HtlvCrcDecoder) Intercept(chain biface.IChain) biface.IcResp {
	//1. Get the IMessage of btgo
	...
	//blogger.Ins().DebugF("HTLVCRC-RawData size:%d data:%s\n", len(data), hex.EncodeToString(data))
	...
	//5. Set the decoded data back to the IMessage, the btgo Router needs MsgID for addressing
	// (将解码后的数据重新设置到IMessage中, btgo的Router需要MsgID来寻址)
