...
func NewTLVDecoder() biface.IDecoder {
	return &TLVDecoder{}
}

func (tlv *TLVDecoder) GetLengthField() *biface.LengthField {
	// +---------------+---------------+---------------+
	// |    Tag        |     Length    |     Value     |
...
func (tlv *TLVDecoder) decode(data []byte) *TLVDecoder {
	...
	//blogger.Ins().DebugF("TLV-DecodeData size:%d data:%+v\n", unsafe.Sizeof(data), tlvData)
	return &tlvData
}

func (tlv *TLVDecoder) Intercept(chain biface.IChain) biface.IcResp {
	...
	//blogger.Ins().DebugF("TLV-RawData size:%d data:%s\n", len(data), hex.EncodeToString(data))
