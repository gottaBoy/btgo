// - stdlogger module, which provides global logging methods
// - logger module, which defines logging protocols as object methods
package blogger
...
const (
	BitDate         = 1 << iota                            // Date flag bit 2023/01/23 (日期标记位)
	BitTime                                                // Time flag bit 01:23:12 (时间标记位)
	BitMicroSeconds                                        // Microsecond flag bit 01:23:12.111222 (微秒级标记位)
	BitLongFile                                            // Complete file name /home/go/src//server.go (完整文件名称)
...
func NewLogger(prefix string, flag int) *Logger {
	...
	// By default, debug is turned on, the depth is 2, and the Logger object calling the log print method can call up to two levels to reach the output function
	// (默认 debug打开， calledDepth深度为2,Logger对象调用日志打印方法最多调用两层到达output函数)
	zlog := &Logger{prefix: prefix, flag: flag, isolationLevel: 0, calldDepth: 2}
...
func (log *Logger) SetLogHook(f func([]byte)) {
	log.onLogHook = f
}
		...
		if log.flag&(BitShortFile|BitLongFile) != 0 {
			...
			if log.flag&BitShortFile != 0 {
				...
				for i := len(file) - 1; i > 0; i-- {
					if file[i] == '/' {
						// Get the file name after the last '/' character, e.g. ".go" from "/home/go/src/.go"
						short = file[i+1:]
						break
					}
				}
...
func (log *Logger) verifyLogIsolation(logLevel int) bool {
...
// SetMaxAge 最大保留天数
func (log *Logger) SetMaxAge(ma int) {
	if log.fw == nil {
		return
	}
	log.mu.Lock()
...
func (log *Logger) SetMaxSize(ms int64) {
	if log.fw == nil {
		return
	}
	log.mu.Lock()
...
// SetCons 同时输出控制台
func (log *Logger) SetCons(b bool) {
	if log.fw == nil {
		return
	}
	log.mu.Lock()
