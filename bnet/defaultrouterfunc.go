...
func RouterRecovery(request biface.IRequest) {
	defer func() {
		if err := recover(); err != nil {
			panicInfo := getInfo(StackBegin)
			// Record the error
			blogger.Ins().ErrorF("MsgId:%d Handler panic: info:%s err:%v", request.GetMsgId(), panicInfo, err)
...
// (简单累计所有路由组的耗时，不启用)
func RouterTime(request biface.IRequest) {
	now := time.Now()
	request.RouterSlicesNext()
