...
func newServerConn(server biface.IServer, conn net.Conn, connId uint64) biface.IConnection {
...
func newClientConn(client biface.IClient, conn net.Conn) biface.IConnection {
	c := &Connection{
		conn:        conn,
	...
	for {
		...
		case data, ok := <-c.msgBuffChan:
			if ok {
				...
				blogger.Ins().ErrorF("msgBuffChan is Closed")
				break
			}
		case <-c.ctx.Done():
			return
...
func (c *Connection) SendToQueue(data []byte) error {
	...
	if c.msgBuffChan == nil {
		c.msgBuffChan = make(chan []byte, bconf.GlobalObject.MaxMsgChanLen)
		...
		// 此方法只读取MsgBuffChan中的数据没调用SendBufMsg可以分配内存和启用协程)
		go c.StartWriter()
	}
...
func (c *Connection) callOnConnStart() {
	if c.onConnStart != nil {
		blogger.Ins().InfoF("o CallOnConnStart....")
		c.onConnStart(c)
	}
}
...
func (c *Connection) SetHeartBeat(checker biface.IHeartbeatChecker) {
	c.hc = checker
}
...
func (c *Connection) GetMsgHandler() biface.IMsgHandler {
	return c.msgHandler
}
