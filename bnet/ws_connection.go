...
type WsConnection struct {
	...
	// msgBufChan is a buffered channel used for message communication between the read and write goroutines.
...
func newWebsocketConn(server biface.IServer, conn *websocket.Conn, connId uint64) biface.IConnection {
...
func newWsClientConn(client biface.IClient, conn *websocket.Conn) biface.IConnection {
	c := &WsConnection{
		conn:       conn,
...
func (c *WsConnection) StartReader() {
	...
	for {
		...
		default:
			...
			n := len(buffer)
			if err != nil {
				blogger.Ins().ErrorF("read msg head [read datalen=%d], error = %s", n, err.Error())
				return
			}
...
func (c *WsConnection) GetTCPConn() net.Conn {
	return nil
}
...
func (c *WsConnection) SendToQueue(data []byte) error {
	...
	if c.msgBufChan == nil {
		...
		// which only reads data from msgBufChan and hasn't allocated memory or started the coroutine until SendBufMsg is called
		// (开启用于写回客户端数据流程的Goroutine
		// 此方法只读取msgBufChan中的数据没调用SendBufMsg可以分配内存和启用协程)
		go c.StartWriter()
	}
...
// SendBufMsg sends BuffMsg
func (c *WsConnection) SendBufMsg(msgId uint32, data []byte) error {
	c.msgLock.RLock()
	defer c.msgLock.RUnlock()
	...
	if c.msgBufChan == nil {
		...
		// This method only reads data from msgBufChan, allocating memory and starting Goroutine without calling SendBufMsg
		// (开启用于写回客户端数据流程的Goroutine
		// 此方法只读取msgBufChan中的数据没调用SendBufMsg可以分配内存和启用协程)
		go c.StartWriter()
	}
...
func (c *WsConnection) SetHeartBeat(checker biface.IHeartbeatChecker) {
	c.hc = checker
}
...
func (c *WsConnection) GetMsgHandler() biface.IMsgHandler {
	return c.msgHandler
}
