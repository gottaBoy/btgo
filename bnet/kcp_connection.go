...
type KcpConnection struct {
	...
	connManager biface.IConnManager
...
func newKcpServerConn(server biface.IServer, conn *kcp.UDPSession, connID uint64) biface.IConnection {
	...
	if lengthField != nil {
		c.frameDecoder = binterceptor.NewFrameDecoder(*lengthField)
	}
...
func newKcpClientConn(client biface.IClient, conn *kcp.UDPSession) biface.IConnection {
	c := &KcpConnection{
		conn:       conn,
...
func (c *KcpConnection) StartWriter() {
	blogger.Ins().InfoF("Writer Goroutine is running")
	defer blogger.Ins().InfoF("%s [conn Writer exit!]", c.RemoteAddr().String())
		...
		case data, ok := <-c.msgBufChan:
			if ok {
				if err := c.Send(data); err != nil {
					blogger.Ins().ErrorF("Send Buff Data error:, %s Conn Writer exit", err)
					break
				}

			} else {
				blogger.Ins().ErrorF("msgBufChan is Closed")
				break
			}
		case <-c.ctx.Done():
			return
...
func (c *KcpConnection) StartReader() {
	blogger.Ins().InfoF("[Reader Goroutine is running]")
	defer blogger.Ins().InfoF("%s [conn Reader exit!]", c.RemoteAddr().String())
	defer c.Stop()
	defer func() {
		if err := recover(); err != nil {
			blogger.Ins().ErrorF("connID=%d, panic err=%v", c.GetConnId(), err)
		}
	}()
	...
	for {
		...
		default:
			...
			buffer := make([]byte, bconf.GlobalObject.IOReadBuffSize)

			// read data from the connection's IO into the memory buffer
			// (从conn的IO中读取数据到内存缓冲buffer中)
			n, err := c.conn.Read(buffer)
			if err != nil {
				blogger.Ins().ErrorF("read msg head [read datalen=%d], error = %s", n, err)
				return
			}
			blogger.Ins().DebugF("read buffer %s \n", hex.EncodeToString(buffer[0:n]))
			...
			if c.frameDecoder != nil {
				...
				for _, bytes := range bufArrays {
					...
					msg := bpack.NewMessage(uint32(len(bytes)), bytes)
				...
				msg := bpack.NewMessage(uint32(n), buffer[0:n])
				// Get the current client's Request data
				// (得到当前客户端请求的Request数据)
				req := NewRequest(c, msg)
				c.msgHandler.Execute(req)
...
func (c *KcpConnection) Start() {
	defer func() {
		if err := recover(); err != nil {
			blogger.Ins().ErrorF("Connection Start() error: %v", err)
		}
	}()
...
func (c *KcpConnection) GetTCPConn() net.Conn {
	return c.conn
}
...
func (c *KcpConnection) Send(data []byte) error {
	...
	if err != nil {
		blogger.Ins().ErrorF("SendMsg err data = %+v, err = %+v", data, err)
		return err
	}
...
func (c *KcpConnection) SendToQueue(data []byte) error {
	...
	if c.msgBufChan == nil {
		c.msgBufChan = make(chan []byte, bconf.GlobalObject.MaxMsgChanLen)
		// Start a Goroutine to write data back to the client
		// This method only reads data from the msgBufChan without allocating memory or starting a Goroutine
		// (开启用于写回客户端数据流程的Goroutine
		// 此方法只读取msgBufChan中的数据没调用SendBuffMsg可以分配内存和启用协程)
		go c.StartWriter()
	}
	...
	if data == nil {
		blogger.Ins().ErrorF("Pack data is nil")
...
func (c *KcpConnection) SendMsg(msgID uint32, data []byte) error {
	...
	msg, err := c.packet.Pack(bpack.NewMsgPackage(msgID, data))
	if err != nil {
		blogger.Ins().ErrorF("Pack error msg ID = %d", msgID)
		return errors.New("Pack error msg ")
	}

	err = c.Send(msg)
	if err != nil {
		blogger.Ins().ErrorF("SendMsg err msg ID = %d, data = %+v, err = %+v", msgID, string(msg), err)
		return err
	}
...
func (c *KcpConnection) SendBufMsg(msgID uint32, data []byte) error {
	if c.isClosed == true {
		return errors.New("connection closed when send buff msg")
	}
	if c.msgBufChan == nil {
...
func (c *KcpConnection) finalizer() {
	...
	blogger.Ins().InfoF("Conn Stop()...ConnID = %d", c.connID)
}

func (c *KcpConnection) callOnConnStart() {
	if c.onConnStart != nil {
		blogger.Ins().InfoF("btgo CallOnConnStart....")
		c.onConnStart(c)
	}
}
...
func (c *KcpConnection) callOnConnStop() {
	if c.onConnStop != nil {
		blogger.Ins().InfoF("btgo CallOnConnStop....")
		c.onConnStop(c)
	}
}
...
func (c *KcpConnection) IsAlive() bool {
	...
	return time.Now().Sub(c.lastActivityTime) < bconf.GlobalObject.HeartbeatMaxDuration()
}
...
func (c *KcpConnection) SetHeartBeat(checker biface.IHeartbeatChecker) {
	c.hc = checker
}
...
func (c *KcpConnection) GetMsgHandler() biface.IMsgHandler {
	return c.msgHandler
}
...
// func newKCPConn(client  biface.IClient, conn net.Conn)  biface.IConnection {
//   c := &Connection{
//     conn:        conn,
//     connID:      0, // client ignore
//     isClosed:    false,
...
// func newServerKCPConn(server  biface.IServer, conn net.Conn, connID uint64)  biface.IConnection {
//   c := &Connection{
//     conn:        conn,
//     connID:      connID,
//     isClosed:    false,
