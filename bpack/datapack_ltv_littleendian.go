...
// DataPackLtv
// LTV little-endian data packing and unpacking used by btgo in its early days, compatible with previous applications
// (btgo早期使用的LTV 小端方式，兼容之前的应用)
type DataPackLtv struct{}

// NewDataPackLtv initializes a packing and unpacking instance
// (封包拆包实例初始化方法)
func NewDataPackLtv() biface.IDataPack {
	return &DataPackLtv{}
}
...
// (封包方法,压缩数据)
func (dp *DataPackLtv) Pack(msg biface.IMessage) ([]byte, error) {
...
// (拆包方法,解压数据)
func (dp *DataPackLtv) UnPack(binaryData []byte) (biface.IMessage, error) {
	...
	if bconf.GlobalObject.MaxPacketSize > 0 && msg.GetDataLen() > bconf.GlobalObject.MaxPacketSize {
